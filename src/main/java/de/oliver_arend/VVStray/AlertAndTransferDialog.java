...
public class AlertAndTransferDialog extends JDialog {
	...
	public AlertAndTransferDialog(ArrayList<String> alerts, ArrayList<String> transfers) {
		...
		alerts.forEach((alert) -> {
			...
			alertPanel.add(new JLabel(alert, new ImageIcon(Utils.getResourcePath("alert.png")), SwingConstants.LEFT));
			this.add(alertPanel);
		});
		if(alerts.size() > 0 && transfers.size() > 0) {
		...
		transfers.forEach((transfer) -> {
			...
			transferPanel.add(new JLabel(transfer, new ImageIcon(Utils.getResourcePath("transfer.png")), SwingConstants.LEFT));
